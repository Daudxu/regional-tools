...
router.get('/sa', function (req, res, next) {
  ...
  arr.forEach((item) => {
    conn.query(`INSERT INTO ls_dev_region ( id, parent_id, LEVEL, NAME, short ) VALUES ( 	${item.value}, ${item.pid}, ${item.level}, ${item.level}, "aaaaaaaaaaaaaa")`, (err, results, fieldes) => {
      if (err) {
        throw err;
      }
    })
