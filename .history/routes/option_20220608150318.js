...
router.get('/sa', function (req, res, next) {
  ...
  arr.forEach((item) => {
    conn.query(`INSERT INTO ls_dev_region ( id, parent_id, LEVEL, NAME, short ) VALUES ( 	${item.value}, ${item.pid}, ${item.level}, "aaaaaaaaaaaaaa", "aaaaaaaaaaaaaa")`, (err, results, fieldes) => {
      if (err) {
        throw err;
      }
    })
  ...
  // })
  // conn.query(`INSERT INTO ls_dev_region(id, parent_id, LEVEL, NAME, short) VALUES(343223123, 343223123, 100, "aaaaaaaaaaaaaa", "aaaaaaaaaaaaaa")`, (err, results, fieldes) => {
  //   if (err) {
  //     throw err;
  //   }
  //   // res.send(results);
